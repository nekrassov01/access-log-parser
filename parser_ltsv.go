...
func NewLTSVParser(ctx context.Context, writer io.Writer, opt Option) *LTSVParser {
	p := &LTSVParser{
		ctx:     ctx,
		writer:  writer,
...
func (p *LTSVParser) Parse(reader io.Reader) (*Result, error) {
	return parse(p.ctx, reader, p.writer, nil, p.decoder, p.opt)
}
...
func (p *LTSVParser) ParseString(s string) (*Result, error) {
	return parseString(p.ctx, s, p.writer, nil, p.decoder, p.opt)
}
...
func (p *LTSVParser) ParseFile(filePath string) (*Result, error) {
	return parseFile(p.ctx, filePath, p.writer, nil, p.decoder, p.opt)
}
...
func (p *LTSVParser) ParseGzip(gzipPath string) (*Result, error) {
	return parseGzip(p.ctx, gzipPath, p.writer, nil, p.decoder, p.opt)
}
...
func (p *LTSVParser) ParseZipEntries(zipPath, globPattern string) (*Result, error) {
	return parseZipEntries(p.ctx, zipPath, globPattern, p.writer, nil, p.decoder, p.opt)
}
