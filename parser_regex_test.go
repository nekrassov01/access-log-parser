...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			writer := &bytes.Buffer{}
			p := NewApacheCLFRegexParser(writer)
			_, err := p.ParseString(tt.parserArgs.input, nil, nil, nil, false)
			if err != nil {
				t.Fatal(err)
			}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			writer := &bytes.Buffer{}
			p := NewApacheCLFWithVHostRegexParser(writer)
			_, err := p.ParseString(tt.parserArgs.input, nil, nil, nil, false)
			if err != nil {
				t.Fatal(err)
			}
